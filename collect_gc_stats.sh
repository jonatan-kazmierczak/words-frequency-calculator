...
/bin/time $JAVA_J9_CMD -Xgcpolicy:nogc -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

echo -n vm=OpenJ9,gc=gencon, >> $OUT_FILE
/bin/time $JAVA_J9_CMD -Xgcpolicy:gencon -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

echo -n vm=OpenJ9,gc=balanced, >> $OUT_FILE
/bin/time $JAVA_J9_CMD -Xgcpolicy:balanced -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

echo -n vm=OpenJ9,gc=optavgpause, >> $OUT_FILE
/bin/time $JAVA_J9_CMD -Xgcpolicy:optavgpause -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

echo -n vm=OpenJ9,gc=optthruput, >> $OUT_FILE
/bin/time $JAVA_J9_CMD -Xgcpolicy:optthruput -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

echo -n vm=OpenJ9,gc=metronome, >> $OUT_FILE
/bin/time $JAVA_J9_CMD -Xgcpolicy:metronome -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE

# Zing

echo -n vm=Zing,gc=C4, >> $OUT_FILE
/bin/time $JAVA_ZING_CMD -jar build/libs/words-freq-calc.jar CalculatorJ8 $TEXT_FILE $REPEAT_COUNT 2>>$OUT_FILE
